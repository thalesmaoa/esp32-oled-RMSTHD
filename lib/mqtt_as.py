...
class MQTT_base:
    ...
    def dprint(self, *args):
        if self.DEBUG:
            print(*args)
            self.led.fill_rect(0, 45, 128, 18, 0)
            self.led.show()
            st_print = ''
            for st in args:
                if type(st) != OSError:
                    st_print += st
            if len(st_print) > 15:
                self.led.text(st_print[:16], 0, 45)
                self.led.text(st_print[16:], 0, 55)
            else:
                self.led.text(st_print, 0, 45)
            self.led.show()
